function [cum_rwd] = PGDeterministic(agent, mdp, iterations, momentum, c_epsilon,  sigma)
...
for p = 2:(iterations+1)
    ...
    Velocity = momentum*Old_Velocity + nu_k * gradient_inc;
    
    eta_k = 1/ (transpose(gradient_inc) * Velocity);
        
    newVariables = current_variables + epsilon * eta_k* Velocity;   
