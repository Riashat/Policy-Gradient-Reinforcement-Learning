function [cum_rwd] = PGStochastic(agent, mdp, iterations,sigma)
...
for p = 2:(iterations+1)
    ...
    new_Traj = [succ_state];
     ...
     [policy_gradient, R] = pg_gradient(agent, critic, rho_d, old_Traj, new_Traj);
...
%adaptive step size here - "Adaptive Step-Size for Policy Gradient Methods"
    ...
    Den_Constant = ( mdp.gamma * sqrt(2*pi) * sigma ) + 2*(1-mdp.gamma)*mdp.action_dim;   
    M=1;  
    Den_Constant2 = R * M.^2;
